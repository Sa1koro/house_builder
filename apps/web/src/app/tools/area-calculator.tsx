...
export function AreaCalculator() {
  ...
  const fields: { label: string; value: number; setValue: (value: number) => void }[] = [
    { label: "档位基础价", value: base, setValue: setBase },
    { label: "档位含面积㎡", value: included, setValue: setIncluded },
    { label: "实际计价面积㎡", value: area, setValue: setArea },
    { label: "超面积单价", value: unit, setValue: setUnit },
  ];
  return (
    <div className="paper rounded-3xl p-6">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        {fields.map(({ label, value, setValue }) => <label key={label} className="text-xs text-[#65736b]">{label}<input type="number" step=".01" value={value} onChange={(event) => setValue(Number(event.target.value))} className="mt-2 w-full rounded-xl bg-[#f5f3ee] px-4 py-3 text-base text-[#18221d] outline-none" /></label>)}
      </div>
